...
`nvidia-persistenced` postinst failed on first install (started the daemon before `nvidia.ko`). Finished with a one-shot `policy-rc.d` exit 101; package is `ii`. Daemon is not required for the gate (Persistence-M Off). `sartoria-nvidia` 0.0.2 ships a preinst so the next install does not hit that.
