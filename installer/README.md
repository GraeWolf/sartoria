...
Offline ISO. Whole disk. After the disk is chosen, the TUI asks whether to encrypt the root with LUKS2. No keeps the v0.1 layout: BIOS boot, EFI, one ext4 root. Yes adds an unencrypted 1G `/boot` and a LUKS2 root. The EFI partition stays unencrypted. The passphrase is not the login password. GRUB loads the kernel from `/boot`, and the initramfs asks for the passphrase once.
