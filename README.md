...
**Status:** Phase F on this G15 (F1 font done; F-suspend 0.0.10 restores the keyboard backlight on wake (`i8042.dumbkbd=1` from 0.0.9; one lid cycle passed); F2 Brave Origin installed; F3 LibreOffice from Excalibur; F4 done: Nautilus, screenshots, and Xft.dpi 144 on this panel; F5 Steam installed (Prime offload, Super+g; sign-in window still tiles); F6 done: LUKS is an installer question, proved on a VM, not this disk; F7 done: optional `sartoria-ai` 0.0.1 installed, off until a key, not on the ISO). Phase E (hybrid Prime) is done. v0.1 ISO remains Phase D. Desktop dGPU is a later follow-up.
...
| Packaging | apt/dpkg, `sartoria-desktop`, `sartoria-nvidia`, `sartoria-origin`, `sartoria-office`, `sartoria-daily`, `sartoria-games`, `sartoria-ai` |
